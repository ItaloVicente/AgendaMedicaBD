...
public class AgendarConsulta extends javax.swing.JFrame {
            ...
            if(consultasMesmoDia.size()>=4){
                    ...
                    try{
                        ...
                        for(Consulta consulta3: consultas){
                            if(consulta3.getHorario().equals(hora)&&consulta3.getData().equals(dataNova)){
                                JOptionPane.showMessageDialog(null, "Olá, infelizmente o médico está com uma consulta nesse horario, se quiser altere a hora.","ERRO", JOptionPane.ERROR_MESSAGE);
                                break;
                            }
                            a++;
                            if(a==tamanho&&verificador2==false){
                                medicos.remove(index);
                    Consulta consulta = new Consulta(nomeMedico,paciente.getNome(),dataNova,hora);
                    medico.adicionarConsulta(consulta);
                ...
                if(tamanho==0){
                    ...
                    for(int t=0;t<pacientes.size();t++){
                        Paciente paciente1 = pacientes.get(t);
                        if(paciente1.getNome().equals(paciente.getNome())&&paciente1.getSenha().equals(paciente.getSenha())){
                            pacientes.remove(index1);
                            paciente1.adicionarConsulta(consulta);
                            pacientes.add(paciente1);
                    ...
                    try{
                        ...
                        writer.close();
                        JOptionPane.showMessageDialog(null, "O agendamento foi marcado com sucesso!");
                    }catch(JsonIOException | HeadlessException | IOException e){
                            e.getMessage();
                    }
                    break;
                }
                ...
                for(int x =0;x<consultas.size();x++){
                    ...
                    a++;
                    if(a==tamanho&&verificador==false){
                        System.out.println("entrou");
                        medicos.remove(index);
                    medico.adicionarConsulta(consulta);
                    paciente.addListaEspera(consulta);
                    int index1=0;
                    for(int y=0;y<pacientes.size();y++){
                        Paciente paciente1 = pacientes.get(y);
                        if(paciente1.getNome().equals(paciente.getNome())&&paciente1.getSenha().equals(paciente.getSenha())){
                            pacientes.remove(index1);
                            paciente1.adicionarConsulta(consulta);
                            pacientes.add(paciente1);
                    ...
                    Writer writer2 = null;
                    try{
                        writer2=new FileWriter("medicos.json");
                        gson.toJson(medicos,writer2);
                        writer2.flush();
                        writer2.close();
                    }catch(JsonIOException | IOException e){
                            e.getMessage();
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        JOptionPane.showMessageDialog(null, "Ao finalizar você precisa logar novamente para que sua lista de agendamentos seja atualizada.");
